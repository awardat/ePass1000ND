...
到上一行为止，我一直想着的是把证书放在令牌上，这样就可以在用不同电脑的时候都可以给邮件签名，而不用把证书安装到每一台电脑上。

想象很丰满结果很骨感，直接在令牌查看证书都是正常的，在outlook里加载也是正常的，签名也是正常的，结果收信的时候发现岔子了，证书被标注成过期或不可信，所以邮件有被篡改的风险。

然后尝试把证书导入Windows使用一切正常，就是说证书在被导入令牌的过程中发生了什么，或者令牌自己做了什么而导致导入的证书失效。